...
<body>
  ...
  <div class="box3">
  	<c:choose>
		<c:when test="${wt.expertnick==null}">
			<img src="img/profile1.png">&nbsp
		...
		<c:when test="${questionList.expertnick!=null}">
			<img src="img/profile2.png">&nbsp
			<span id="span1">${wt.expertnick}</span>
			<span id="span2">${wt.postdate}</span>
			<button type="button" id="btn1">+&nbsp팔로우</button>
		</c:when>
	</c:choose>
  ...
  <c:choose>
		<c:when test="${wt.expertnick==null}">
			<c:if test="${wt.nick==connectNick}">
			 <div style="display: flex; justify-content : center; text-align:left;">
  			 <div class="box5">
    		 <span>수정</span>&nbsp|&nbsp<span>삭제</span>
  			 </div>
  			 </div>
  			 </c:if>
		</c:when>
		<c:when test="${questionList.expertnick!=null}">
			<c:if test="${wt.expertnick==connectNick}">
			 <div style="display: flex; justify-content : center; text-align:left;">
  			 <div class="box5">
    		 <span>수정</span>&nbsp|&nbsp<span>삭제</span>
  			 </div>
